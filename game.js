// All questions, options and answers in json array format
let questionBank = [{
    ...
    "question": "In the movie 'Austin: The Spy Who Shagged Me' what is the name of Dr. Evil's diminitive clone?",
    "A": "Little Buddy",
    "B": "Mini-Me",
    "C": "Small Fry",
    "D": "Dr. Evil Jr",
...
//score
let total = 0;
//which question to select from the array
let num = 0;
...
function quiz() {
    ...
    //select question para
    var question = document.getElementById("question");
    var questionNumber = document.getElementById("questionNumber");

    // update text
    question.innerHTML = questionBank[num].question;
    questionNumber.innerHTML = quesNumber;

    console.log("Answer " + questionBank[num].answer);

    //get each label and update options text
    let options = document.querySelector('label[for="radio' + 0 + '"]');
    options.innerHTML = questionBank[num].A;

    options = document.querySelector('label[for="radio' + 1 + '"]');
    options.innerHTML = questionBank[num].B;

    options = document.querySelector('label[for="radio' + 2 + '"]');
    options.innerHTML = questionBank[num].C;

    options = document.querySelector('label[for="radio' + 3 + '"]');
    options.innerHTML = questionBank[num].D;

}

function submitQuiz() {
    //Get selected radio button by the user
    const selectedOption = document.querySelector('input[name="ques"]:checked');
    //see if user selected any button or not
    if (selectedOption) {
        //if yes then check the answer is correct or not
        let ans = questionBank[num].answer;
        if ((selectedOption.className) === ans) {
            total++;
            quesNumber++;
